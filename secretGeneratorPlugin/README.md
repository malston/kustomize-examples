...
go get sigs.k8s.io/kustomize/api/resmap
go get sigs.k8s.io/kustomize/api/types
go get sigs.k8s.io/kustomize/api/kv
...
result=$( \
  ...
  kustomize build --enable-alpha-plugins $DEMO_HOME )
echo "$result"
# Spot check the result:
test 1 == $(echo "$result" | grep -c "FRUIT: YXBwbGU=")
```
